/*
 ...
 1 motor linear control with Omni3MD board;
...
void loop()
{
    ...
    omni.mov_lin1m_pid(M1,speed1);    // move motor1 at speed1
    delay(1);                         // wait 1ms for Omni3MD to process information
    omni.mov_lin1m_pid(M2,speed2);    // move motor2 at speed2
    delay(1);                         // wait 1ms for Omni3MD to process information
    omni.mov_lin1m_pid(M3,speed3);    // move motor3 at speed3
    delay(1);                         // wait 1ms for Omni3MD to process information
    
    wait(20); //wait 4 seconds while printing in the serial monitor
 
    omni.stop_motors();       // stop all motors without holding torque
    wait(10); //wait 2 seconds while printing in the serial monitor  

    omni.mov_lin1m_pid(M1,-speed3);   // move motor1 at speed1
    delay(1);                         // wait 1ms for Omni3MD to process information
    omni.mov_lin1m_pid(M2,-speed3);   // move motor2 at speed2
    delay(1);                         // wait 1ms for Omni3MD to process information
    omni.mov_lin1m_pid(M3,-speed3);   // move motor3 at speed3
    delay(1);                         // wait 1ms for Omni3MD to process information
    ...
    while(enc1>0 || enc2>0 || enc3>0)
    {
        if(enc1<0)
        {
            omni.mov_lin1m_pid(M1,0);       // stop motor with holding torque
            delay(1);                       // wait 1ms for Omni3MD to process information
        }
        if(enc2<0)
        {
            omni.mov_lin1m_pid(M2,0);       // stop motor with holding torque
            delay(1);                       // wait 1ms for Omni3MD to process information
        }
        if(enc3<0)
        {
            omni.mov_lin1m_pid(M3,0);       // stop motor with holding torque
            delay(1);                       // wait 1ms for Omni3MD to process information
        }     
        omni.read_encoders(&enc1,&enc2,&enc3); // read all encoders at once (in a single I2C request) 
        print_enc_values(); //Print values in serial monitor
    ...
    wait(40); //wait 2 seconds while printing in the serial monitor 
}
