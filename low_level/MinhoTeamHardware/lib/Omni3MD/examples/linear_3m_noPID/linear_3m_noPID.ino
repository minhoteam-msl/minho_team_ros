/*
 ...
 3 motor linear motion with Omni3MD board;
...
This example demonstrates linear movimentation of 3 motors without PID control
 ...
 Send movimentation orders: 
   Function: mov_lin3m_nopid( speed1, speed2 , speed3 )
   ...
   Ex: omni.mov_lin3m_nopid(35,-45,55);
...
void loop()
{
    ...
    omni.mov_lin3m_nopid(speed1,speed2,speed3);    // move motors at desired speed
    delay(4000);
    
    omni.stop_motors();       // stop all motors instantly
    delay(2000);

    omni.mov_lin3m_nopid(-speed1,-speed2,-speed3);    // move motors at desired speed
    delay(4000);
 
    omni.mov_lin3m_nopid(0,0,0);    // stop motors (with acceleration ramp)
    delay(2000);
}
