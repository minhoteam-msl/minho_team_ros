...
RoleDefender::RoleDefender() : Role(rDEFENDER)
{
...
void RoleDefender::setRosNodeHandle(ros::NodeHandle *parent, std::string topic_base)
{

}

void RoleDefender::determineAction()
{
   ...
   switch(mBsInfo.gamestate){
      ...
      static int invisCounter = 0;
         ...
         if(mRobot.has_ball){
            ...
            mAction = aENGAGEBALL;
            ai->target_pose = mRobot.ball_position;
            ai->target_pose.z = orientationToTarget(mRobot.ball_position.x,mRobot.ball_position.y);
        } else if(invisCounter>200){
            if(mBsInfo.posxside){
                ai->target_pose.x = big_area_x;
